...
static SS4S_AudioFeedResult FeedAudio(SS4S_AudioInstance *instance, const unsigned char *data, size_t size) {
    // Lock-free горячий путь: mediaLoaded — стабильный флаг после OpenAudio,
    // NDL_DirectAudioPlay потокобезопасен внутри webOS.
    ...
    int rc;
    if (context->opusFix) {
        // OpusFix использует внутренний буфер — нужна синхронизация
        pthread_mutex_lock(&SS4S_NDL_webOS5_Lock);
        ...
        size = fixedSize;
        uint64_t pts = SS4S_NDL_webOS5_GetPts(context) + (SS4S_webOS_Is_HighEnd_SoC() ? 8 : 12);
